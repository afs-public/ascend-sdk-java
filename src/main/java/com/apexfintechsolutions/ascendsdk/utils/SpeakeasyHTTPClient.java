...
public class SpeakeasyHTTPClient implements HTTPClient {
   ...
   * including bodies for JSON content. WARNING: this setting may expose sensitive information in
   * logs (like <i>Authorization</i> headers), and should only be enabled temporarily for local
   * debugging purposes. By default, <i>Authorization</i> headers are redacted in the logs ( printed
   * with a value of {@code [*******]}). Header suppression is controlled with the {@link
  ...
  /**
   * Experimental, may be changed anytime. When debug logging is enabled this method controls the
   * suppression of header values in the logs. By default, <i>Authorization</i> headers are redacted
   * in the logs (printed with a value of {@code [*******]}). Header suppression is controlled with
   * the {@link #setRedactedHeaders(Collection)} method.
   *
  ...
  private static HttpResponse<InputStream> logResponse(HttpResponse<InputStream> response)
      throws IOException {
