 ...
 */
package com.apexfintechsolutions.ascendsdk.utils.reactive;
...
public final class ReactiveUtils {
  ...
  /**
   * Maps elements from a Flow.Publisher using an asynchronous transformation function. This is
   * useful for transforming HttpResponse<Blob> elements using operations that return
  ...
  public static <T, R> Flow.Publisher<R> flatten(
      Flow.Publisher<T> source, Function<T, Iterable<R>> flattener) {
  ...
  private static final class ConcatPublisher<T> implements Flow.Publisher<T> {
    ...
    public void subscribe(Flow.Subscriber<? super T> downstream) {
      downstream.onSubscribe(
          new ConcatSubscription<>((Flow.Subscriber<T>) downstream, pubs.iterator()));
    }
  }
  ...
  private static class AsyncMappingSubscriber<T, R> implements Flow.Subscriber<T> {
    ...
    public AsyncMappingSubscriber(
        Flow.Subscriber<? super R> downstream, Function<T, CompletableFuture<R>> mapper) {
  ...
  private static class FlatteningSubscriber<T, R> implements Flow.Subscriber<T> {
    ...
    public FlatteningSubscriber(
        Flow.Subscriber<? super R> downstream, Function<T, Iterable<R>> flattener) {
    ...
    @Override
    public void onSubscribe(Flow.Subscription subscription) {
      this.upstreamSubscription =
          Objects.requireNonNull(subscription, "Upstream subscription cannot be null");
      downstream.onSubscribe(
          new Flow.Subscription() {
            @Override
            public void request(long n) {
