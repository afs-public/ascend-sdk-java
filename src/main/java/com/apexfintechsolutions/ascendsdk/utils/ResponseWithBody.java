...
/**
 * A wrapper for {@link HttpResponse} that allows mappi ng the response body from type {@code R} to
 * type {@code B}.
 *
 * <p>This class delegates all methods to the original response, except for {@link #body()}, which
 * returns the mapped body. The mapping is performed using the provided {@code bodyMapper} function
...
public class ResponseWithBody<R, B> implements HttpResponse<B> {
  ...
  /**
   * Constructs a new {@code ResponseWithBody} by wrapping an existing {@link HttpResponse} and
   * applying a mapping function to its body.
   *
  ...
  /**
   * Constructs a new {@code ResponseWithBody} by wrapping an existing {@link HttpResponse} with a
   * pre-computed body value.
   *
   * @param original the original response to wrap
