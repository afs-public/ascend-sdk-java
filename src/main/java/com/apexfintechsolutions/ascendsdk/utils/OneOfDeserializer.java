...
import com.apexfintechsolutions.ascendsdk.utils.Utils.TypeReferenceWithShape;
...
public class OneOfDeserializer<T> extends StdDeserializer<T> {
   ...
   * @param strict if true then when multiple matches encountered an exception is thrown. If false
   *     then when multiple matches encountered the first match from the typeReferences array is
   *     used as deserialization result.
   * @param typeReferences the types of the oneOf subschemas. When strict is false the first
   *     matching member of this array will be the type of the deserialization result.
   */
  ...
  private static <T> T deserializeOneOf(
      ...
      List<TypeReferenceWithShape> typeReferences,
      Class<T> cls,
      boolean strict)
      throws IOException {
  ...
  private static <T> T deserializeOneOf(
    ...
    if (matches.size() == 1) {
      ...
      throw JsonMappingException.from(
          ctxt,
          "json did not match any of the possible type references: "
              + typeReferenceNames(typeReferences)
              + ", json=\n"
              + json);
    }
  }
  ...
  private static final Set<String> DATE_TIME_CLASSES =
      Set.of(OffsetDateTime.class.getCanonicalName(), LocalDate.class.getCanonicalName());
