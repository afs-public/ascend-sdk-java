 ...
 */
package com.apexfintechsolutions.ascendsdk.utils.pagination;

import com.apexfintechsolutions.ascendsdk.utils.CopiableInputStream;
import com.apexfintechsolutions.ascendsdk.utils.ResponseWithBody;
 ...
 * This class implements the Iterator interface and can work with any request and response types. It
 * uses a ProgressTrackerStrategy to process pagination metadata from responses and determine when
 * to stop pagination.
 *
 * <p>The pagination flow works as follows: 1. hasNext() checks if there's a current response or
 * tries to fetch the next page 2. fetchNext() builds the request using the output handler's current
 * value 3. The response is processed by the progress tracker to update pagination state 4. next()
 * returns the current response and clears it for the next iteration
 *
...
public class Paginator<ReqT, ProgressParamT> implements Iterator<HttpResponse<InputStream>> {
  ...
  /**
   * Returns the current response if one exists. This is useful for peeking at the current response
  ...
  /**
   * Checks if there are more pages to fetch. If there's no current response, it attempts to fetch
   * the next page. The output handler determines if there are more pages based on the response
   * metadata.
   *
  ...
  /**
   * Fetches the next page of data. This method: 1. Builds the appropriate request using the output
   * handler's current value 2. Fetches the data using the provided fetcher function 3. Parses the
   * response using JsonPath 4. Updates output tracker state 5. Updates pagination state based on
  ...
  /**
   * Returns the next page of data. This method returns the current response and clears it, allowing
   * the next call to hasNext() to fetch the next page if needed.
   *
