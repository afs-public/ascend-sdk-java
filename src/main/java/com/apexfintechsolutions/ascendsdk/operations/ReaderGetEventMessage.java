...
public class ReaderGetEventMessage {
  ...
  abstract static class Base {
    ...
    <T> HttpRequest buildRequest(T request, Class<T> klass) throws Exception {
      String url =
          Utils.generateURL(klass, this.baseUrl, "/events/v1/messages/{message_id}", request, null);
  ...
  public static class Sync extends Base
      implements RequestOperation<ReaderGetEventMessageRequest, ReaderGetEventMessageResponse> {
    public Sync(SDKConfiguration sdkConfiguration, Optional<Options> options) {
      super(sdkConfiguration, options);
    }

    private HttpRequest onBuildRequest(ReaderGetEventMessageRequest request) throws Exception {
      HttpRequest req = buildRequest(request, ReaderGetEventMessageRequest.class);
    ...
    @Override
    public HttpResponse<InputStream> doRequest(ReaderGetEventMessageRequest request)
        throws Exception {
      Retries retries =
          Retries.builder()
              .action(
    ...
    @Override
    public ReaderGetEventMessageResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      ReaderGetEventMessageResponse.Builder resBuilder =
          ReaderGetEventMessageResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      ReaderGetEventMessageResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          EventMessage out = Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withEventMessage(out);
          return res;
        } else {
          throw new SDKError(
              response,
