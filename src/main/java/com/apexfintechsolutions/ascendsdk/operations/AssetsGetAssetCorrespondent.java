...
public class AssetsGetAssetCorrespondent {
  ...
  public static class Sync extends Base
      implements RequestOperation<
          AssetsGetAssetCorrespondentRequest, AssetsGetAssetCorrespondentResponse> {
    public Sync(SDKConfiguration sdkConfiguration, Optional<Options> options) {
      super(sdkConfiguration, options);
    }

    private HttpRequest onBuildRequest(AssetsGetAssetCorrespondentRequest request)
        throws Exception {
      HttpRequest req = buildRequest(request, AssetsGetAssetCorrespondentRequest.class);
    ...
    @Override
    public HttpResponse<InputStream> doRequest(AssetsGetAssetCorrespondentRequest request)
        throws Exception {
      Retries retries =
          Retries.builder()
              .action(
    ...
    @Override
    public AssetsGetAssetCorrespondentResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      AssetsGetAssetCorrespondentResponse.Builder resBuilder =
          AssetsGetAssetCorrespondentResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      AssetsGetAssetCorrespondentResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          Asset out = Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withAsset(out);
          return res;
        } else {
          throw new SDKError(
