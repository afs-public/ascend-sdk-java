...
public class AccountTransfersListTransfers {
  ...
  public static class Sync extends Base
      implements RequestOperation<
          AccountTransfersListTransfersRequest, AccountTransfersListTransfersResponse> {
    public Sync(SDKConfiguration sdkConfiguration, Optional<Options> options) {
      super(sdkConfiguration, options);
    }

    private HttpRequest onBuildRequest(AccountTransfersListTransfersRequest request)
        throws Exception {
      HttpRequest req = buildRequest(request, AccountTransfersListTransfersRequest.class);
    ...
    @Override
    public HttpResponse<InputStream> doRequest(AccountTransfersListTransfersRequest request)
        throws Exception {
      Retries retries =
          Retries.builder()
              .action(
    ...
    @Override
    public AccountTransfersListTransfersResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      AccountTransfersListTransfersResponse.Builder resBuilder =
          AccountTransfersListTransfersResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      AccountTransfersListTransfersResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          ListTransfersResponse out =
              Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withListTransfersResponse(out);
          return res;
        } else {
          throw new SDKError(
              response,
