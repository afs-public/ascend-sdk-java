...
public class AccountTransfersRejectTransfer {
    ...
    <T, U> HttpRequest buildRequest(T request, Class<T> klass, TypeReference<U> typeReference)
      ...
      SerializedBody serializedRequestBody =
          Utils.serializeRequestBody(
              convertedRequest, "rejectTransferRequestCreate", "json", false);
  ...
  public static class Sync extends Base
      implements RequestOperation<
          AccountTransfersRejectTransferRequest, AccountTransfersRejectTransferResponse> {
    public Sync(SDKConfiguration sdkConfiguration, Optional<Options> options) {
      super(sdkConfiguration, options);
    }

    private HttpRequest onBuildRequest(AccountTransfersRejectTransferRequest request)
        throws Exception {
      HttpRequest req =
          buildRequest(
              request,
              AccountTransfersRejectTransferRequest.class,
              new TypeReference<AccountTransfersRejectTransferRequest>() {});
    ...
    @Override
    public HttpResponse<InputStream> doRequest(AccountTransfersRejectTransferRequest request)
        throws Exception {
      Retries retries =
          Retries.builder()
              .action(
    ...
    @Override
    public AccountTransfersRejectTransferResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      AccountTransfersRejectTransferResponse.Builder resBuilder =
          AccountTransfersRejectTransferResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      AccountTransfersRejectTransferResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          RejectTransferResponse out =
              Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withRejectTransferResponse(out);
          return res;
        } else {
          throw new SDKError(
              response,
