...
public class LedgerGetEntry {
  ...
  public static class Sync extends Base
      implements RequestOperation<LedgerGetEntryRequest, LedgerGetEntryResponse> {
    public Sync(SDKConfiguration sdkConfiguration, Optional<Options> options) {
      super(sdkConfiguration, options);
    }

    private HttpRequest onBuildRequest(LedgerGetEntryRequest request) throws Exception {
      HttpRequest req = buildRequest(request, LedgerGetEntryRequest.class);
    ...
    @Override
    public HttpResponse<InputStream> doRequest(LedgerGetEntryRequest request) throws Exception {
      Retries retries =
          Retries.builder()
              .action(
                  () -> {
    ...
    @Override
    public LedgerGetEntryResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      LedgerGetEntryResponse.Builder resBuilder =
          LedgerGetEntryResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      LedgerGetEntryResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          Entry out = Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withEntry(out);
          return res;
        } else {
          throw new SDKError(
