...
public class BookingCreateTrade {
  ...
  abstract static class Base {
    ...
    <T, U> HttpRequest buildRequest(T request, Class<T> klass, TypeReference<U> typeReference)
      ...
      String url =
          Utils.generateURL(
              klass, this.baseUrl, "/booking/v1/accounts/{account_id}/trades", request, null);
      ...
      SerializedBody serializedRequestBody =
          Utils.serializeRequestBody(convertedRequest, "tradeCreate", "json", false);
  ...
  public static class Sync extends Base
      implements RequestOperation<BookingCreateTradeRequest, BookingCreateTradeResponse> {
    public Sync(SDKConfiguration sdkConfiguration, Optional<Options> options) {
      super(sdkConfiguration, options);
    }

    private HttpRequest onBuildRequest(BookingCreateTradeRequest request) throws Exception {
      HttpRequest req =
          buildRequest(
              request,
              BookingCreateTradeRequest.class,
              new TypeReference<BookingCreateTradeRequest>() {});
    ...
    @Override
    public HttpResponse<InputStream> doRequest(BookingCreateTradeRequest request) throws Exception {
      Retries retries =
          Retries.builder()
              .action(
                  () -> {
    ...
    @Override
    public BookingCreateTradeResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      BookingCreateTradeResponse.Builder resBuilder =
          BookingCreateTradeResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      BookingCreateTradeResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          BookingTrade out = Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withBookingTrade(out);
          return res;
        } else {
          throw new SDKError(
              response,
