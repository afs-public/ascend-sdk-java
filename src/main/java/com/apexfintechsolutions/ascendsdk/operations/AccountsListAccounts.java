...
public class AccountsListAccounts {
  ...
  abstract static class Base {
    ...
    HttpRequest buildRequest(AccountsListAccountsRequest request) throws Exception {
      String url = Utils.generateURL(this.baseUrl, "/accounts/v1/accounts");
      HTTPRequest req = new HTTPRequest(url, "GET");
      req.addHeader("Accept", "application/json")
          .addHeader("user-agent", SDKConfiguration.USER_AGENT);

      req.addQueryParams(Utils.getQueryParams(AccountsListAccountsRequest.class, request, null));
  ...
  public static class Sync extends Base
      implements RequestOperation<AccountsListAccountsRequest, AccountsListAccountsResponse> {
    ...
    @Override
    public HttpResponse<InputStream> doRequest(AccountsListAccountsRequest request)
        throws Exception {
    ...
    @Override
    public AccountsListAccountsResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      AccountsListAccountsResponse.Builder resBuilder =
          AccountsListAccountsResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      AccountsListAccountsResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          ListAccountsResponse out =
              Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withListAccountsResponse(out);
          return res;
        } else {
          throw new SDKError(
              response,
