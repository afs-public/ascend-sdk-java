...
public class AccountsDeactivateEnrollment {
    ...
    <T, U> HttpRequest buildRequest(T request, Class<T> klass, TypeReference<U> typeReference)
      ...
      SerializedBody serializedRequestBody =
          Utils.serializeRequestBody(
              convertedRequest, "deactivateEnrollmentRequestCreate", "json", false);
  ...
  public static class Sync extends Base
      implements RequestOperation<
          AccountsDeactivateEnrollmentRequest, AccountsDeactivateEnrollmentResponse> {
    public Sync(SDKConfiguration sdkConfiguration, Optional<Options> options) {
      super(sdkConfiguration, options);
    }

    private HttpRequest onBuildRequest(AccountsDeactivateEnrollmentRequest request)
        throws Exception {
      HttpRequest req =
          buildRequest(
              request,
              AccountsDeactivateEnrollmentRequest.class,
              new TypeReference<AccountsDeactivateEnrollmentRequest>() {});
    ...
    @Override
    public HttpResponse<InputStream> doRequest(AccountsDeactivateEnrollmentRequest request)
        throws Exception {
      Retries retries =
          Retries.builder()
              .action(
    ...
    @Override
    public AccountsDeactivateEnrollmentResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      AccountsDeactivateEnrollmentResponse.Builder resBuilder =
          AccountsDeactivateEnrollmentResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      AccountsDeactivateEnrollmentResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          Enrollment out = Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withEnrollment(out);
          return res;
        } else {
          throw new SDKError(
              response,
