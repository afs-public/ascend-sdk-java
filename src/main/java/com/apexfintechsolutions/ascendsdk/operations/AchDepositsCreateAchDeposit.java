...
public class AchDepositsCreateAchDeposit {
  ...
  abstract static class Base {
    ...
    <T, U> HttpRequest buildRequest(T request, Class<T> klass, TypeReference<U> typeReference)
      ...
      SerializedBody serializedRequestBody =
          Utils.serializeRequestBody(convertedRequest, "achDepositCreate", "json", false);
  ...
  public static class Sync extends Base
      implements RequestOperation<
          AchDepositsCreateAchDepositRequest, AchDepositsCreateAchDepositResponse> {
    public Sync(SDKConfiguration sdkConfiguration, Optional<Options> options) {
      super(sdkConfiguration, options);
    }

    private HttpRequest onBuildRequest(AchDepositsCreateAchDepositRequest request)
        throws Exception {
      HttpRequest req =
          buildRequest(
              request,
              AchDepositsCreateAchDepositRequest.class,
              new TypeReference<AchDepositsCreateAchDepositRequest>() {});
    ...
    @Override
    public HttpResponse<InputStream> doRequest(AchDepositsCreateAchDepositRequest request)
        throws Exception {
      Retries retries =
          Retries.builder()
              .action(
    ...
    @Override
    public AchDepositsCreateAchDepositResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      AchDepositsCreateAchDepositResponse.Builder resBuilder =
          AchDepositsCreateAchDepositResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      AchDepositsCreateAchDepositResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          AchDeposit out = Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withAchDeposit(out);
          return res;
        } else {
          throw new SDKError(
              response,
