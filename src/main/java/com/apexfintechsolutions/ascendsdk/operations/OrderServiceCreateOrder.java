...
public class OrderServiceCreateOrder {
  ...
  abstract static class Base {
    ...
    <T, U> HttpRequest buildRequest(T request, Class<T> klass, TypeReference<U> typeReference)
      ...
      SerializedBody serializedRequestBody =
          Utils.serializeRequestBody(convertedRequest, "orderCreate", "json", false);
  ...
  public static class Sync extends Base
      implements RequestOperation<OrderServiceCreateOrderRequest, OrderServiceCreateOrderResponse> {
    public Sync(SDKConfiguration sdkConfiguration, Optional<Options> options) {
      super(sdkConfiguration, options);
    }

    private HttpRequest onBuildRequest(OrderServiceCreateOrderRequest request) throws Exception {
      HttpRequest req =
          buildRequest(
              request,
              OrderServiceCreateOrderRequest.class,
              new TypeReference<OrderServiceCreateOrderRequest>() {});
    ...
    @Override
    public HttpResponse<InputStream> doRequest(OrderServiceCreateOrderRequest request)
        throws Exception {
      Retries retries =
          Retries.builder()
              .action(
    ...
    @Override
    public OrderServiceCreateOrderResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      OrderServiceCreateOrderResponse.Builder resBuilder =
          OrderServiceCreateOrderResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      OrderServiceCreateOrderResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          Order out = Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withOrder(out);
          return res;
        } else {
          throw new SDKError(
