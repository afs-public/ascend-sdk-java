...
public class AccountsCreateNote {
  ...
  abstract static class Base {
    ...
    <T, U> HttpRequest buildRequest(T request, Class<T> klass, TypeReference<U> typeReference)
      ...
      String url =
          Utils.generateURL(
              klass, this.baseUrl, "/accounts/v1/accounts/{account_id}/notes", request, null);
      ...
      SerializedBody serializedRequestBody =
          Utils.serializeRequestBody(convertedRequest, "noteCreate", "json", false);
  ...
  public static class Sync extends Base
      implements RequestOperation<AccountsCreateNoteRequest, AccountsCreateNoteResponse> {
    public Sync(SDKConfiguration sdkConfiguration, Optional<Options> options) {
      super(sdkConfiguration, options);
    }

    private HttpRequest onBuildRequest(AccountsCreateNoteRequest request) throws Exception {
      HttpRequest req =
          buildRequest(
              request,
              AccountsCreateNoteRequest.class,
              new TypeReference<AccountsCreateNoteRequest>() {});
    ...
    @Override
    public HttpResponse<InputStream> doRequest(AccountsCreateNoteRequest request) throws Exception {
      Retries retries =
          Retries.builder()
              .action(
                  () -> {
    ...
    @Override
    public AccountsCreateNoteResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      AccountsCreateNoteResponse.Builder resBuilder =
          AccountsCreateNoteResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      AccountsCreateNoteResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          Note out = Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withNote(out);
          return res;
        } else {
          throw new SDKError(
