...
public class AccountsCreateRestriction {
  ...
  abstract static class Base {
    ...
    <T, U> HttpRequest buildRequest(T request, Class<T> klass, TypeReference<U> typeReference)
      ...
      SerializedBody serializedRequestBody =
          Utils.serializeRequestBody(convertedRequest, "restrictionCreate", "json", false);
  ...
  public static class Sync extends Base
      implements RequestOperation<
          AccountsCreateRestrictionRequest, AccountsCreateRestrictionResponse> {
    public Sync(SDKConfiguration sdkConfiguration, Optional<Options> options) {
      super(sdkConfiguration, options);
    }

    private HttpRequest onBuildRequest(AccountsCreateRestrictionRequest request) throws Exception {
      HttpRequest req =
          buildRequest(
              request,
              AccountsCreateRestrictionRequest.class,
              new TypeReference<AccountsCreateRestrictionRequest>() {});
    ...
    @Override
    public HttpResponse<InputStream> doRequest(AccountsCreateRestrictionRequest request)
        throws Exception {
      Retries retries =
          Retries.builder()
              .action(
                  () -> {
                    ...
                    try {
                      ...
                      if (Utils.statusCodeMatches(
                          httpRes.statusCode(), "400", "403", "409", "4XX", "500", "503", "5XX")) {
    ...
    @Override
    public AccountsCreateRestrictionResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      AccountsCreateRestrictionResponse.Builder resBuilder =
          AccountsCreateRestrictionResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      AccountsCreateRestrictionResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          Restriction out = Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withRestriction(out);
          return res;
        } else {
          throw new SDKError(
              response,
