...
public class BookingRebookTrade {
  ...
  abstract static class Base {
    ...
    <T, U> HttpRequest buildRequest(T request, Class<T> klass, TypeReference<U> typeReference)
      ...
      SerializedBody serializedRequestBody =
          Utils.serializeRequestBody(convertedRequest, "rebookTradeRequestCreate", "json", false);
  ...
  public static class Sync extends Base
      implements RequestOperation<BookingRebookTradeRequest, BookingRebookTradeResponse> {
    public Sync(SDKConfiguration sdkConfiguration, Optional<Options> options) {
      super(sdkConfiguration, options);
    }

    private HttpRequest onBuildRequest(BookingRebookTradeRequest request) throws Exception {
      HttpRequest req =
          buildRequest(
              request,
              BookingRebookTradeRequest.class,
              new TypeReference<BookingRebookTradeRequest>() {});
    ...
    @Override
    public HttpResponse<InputStream> doRequest(BookingRebookTradeRequest request) throws Exception {
      Retries retries =
          Retries.builder()
              .action(
                  () -> {
    ...
    @Override
    public BookingRebookTradeResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      BookingRebookTradeResponse.Builder resBuilder =
          BookingRebookTradeResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      BookingRebookTradeResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          RebookTradeResponse out =
              Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withRebookTradeResponse(out);
          return res;
        } else {
          throw new SDKError(
              response,
