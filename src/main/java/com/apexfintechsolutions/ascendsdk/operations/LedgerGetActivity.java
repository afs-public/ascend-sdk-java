...
public class LedgerGetActivity {
  ...
  public static class Sync extends Base
      implements RequestOperation<LedgerGetActivityRequest, LedgerGetActivityResponse> {
    public Sync(SDKConfiguration sdkConfiguration, Optional<Options> options) {
      super(sdkConfiguration, options);
    }

    private HttpRequest onBuildRequest(LedgerGetActivityRequest request) throws Exception {
      HttpRequest req = buildRequest(request, LedgerGetActivityRequest.class);
    ...
    @Override
    public HttpResponse<InputStream> doRequest(LedgerGetActivityRequest request) throws Exception {
      Retries retries =
          Retries.builder()
              .action(
                  () -> {
    ...
    @Override
    public LedgerGetActivityResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      LedgerGetActivityResponse.Builder resBuilder =
          LedgerGetActivityResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      LedgerGetActivityResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          Activity out = Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withActivity(out);
          return res;
        } else {
          throw new SDKError(
              response,
