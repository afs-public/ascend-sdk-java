...
public class CashJournalsCheckPartyType {
  ...
  public static class Sync extends Base
    ...
    @Override
    public CashJournalsCheckPartyTypeResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      CashJournalsCheckPartyTypeResponse.Builder resBuilder =
          CashJournalsCheckPartyTypeResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      CashJournalsCheckPartyTypeResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          CheckPartyTypeResponse out =
              Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withCheckPartyTypeResponse(out);
          return res;
        } else {
          throw new SDKError(
              response,
