...
public class OrderPriceServiceRetrieveQuote {
  ...
  abstract static class Base {
    ...
    <T, U> HttpRequest buildRequest(T request, Class<T> klass, TypeReference<U> typeReference)
      ...
      SerializedBody serializedRequestBody =
          Utils.serializeRequestBody(convertedRequest, "retrieveQuoteRequestCreate", "json", false);
  ...
  public static class Sync extends Base
      implements RequestOperation<
          OrderPriceServiceRetrieveQuoteRequest, OrderPriceServiceRetrieveQuoteResponse> {
    public Sync(SDKConfiguration sdkConfiguration, Optional<Options> options) {
      super(sdkConfiguration, options);
    }

    private HttpRequest onBuildRequest(OrderPriceServiceRetrieveQuoteRequest request)
        throws Exception {
      HttpRequest req =
          buildRequest(
              request,
              OrderPriceServiceRetrieveQuoteRequest.class,
              new TypeReference<OrderPriceServiceRetrieveQuoteRequest>() {});
    ...
    @Override
    public HttpResponse<InputStream> doRequest(OrderPriceServiceRetrieveQuoteRequest request)
        throws Exception {
      Retries retries =
          Retries.builder()
              .action(
    ...
    @Override
    public OrderPriceServiceRetrieveQuoteResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      OrderPriceServiceRetrieveQuoteResponse.Builder resBuilder =
          OrderPriceServiceRetrieveQuoteResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      OrderPriceServiceRetrieveQuoteResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          RetrieveQuoteResponse out =
              Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withRetrieveQuoteResponse(out);
          return res;
        } else {
          throw new SDKError(
              response,
