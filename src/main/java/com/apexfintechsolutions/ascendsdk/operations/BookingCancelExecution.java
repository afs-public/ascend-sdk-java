...
public class BookingCancelExecution {
    ...
    <T, U> HttpRequest buildRequest(T request, Class<T> klass, TypeReference<U> typeReference)
      ...
      SerializedBody serializedRequestBody =
          Utils.serializeRequestBody(
              convertedRequest, "cancelExecutionRequestCreate", "json", false);
  ...
  public static class Sync extends Base
      implements RequestOperation<BookingCancelExecutionRequest, BookingCancelExecutionResponse> {
    public Sync(SDKConfiguration sdkConfiguration, Optional<Options> options) {
      super(sdkConfiguration, options);
    }

    private HttpRequest onBuildRequest(BookingCancelExecutionRequest request) throws Exception {
      HttpRequest req =
          buildRequest(
              request,
              BookingCancelExecutionRequest.class,
              new TypeReference<BookingCancelExecutionRequest>() {});
    ...
    @Override
    public HttpResponse<InputStream> doRequest(BookingCancelExecutionRequest request)
        throws Exception {
      Retries retries =
          Retries.builder()
              .action(
    ...
    @Override
    public BookingCancelExecutionResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      BookingCancelExecutionResponse.Builder resBuilder =
          BookingCancelExecutionResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      BookingCancelExecutionResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          CancelExecutionResponse out =
              Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withCancelExecutionResponse(out);
          return res;
        } else {
          throw new SDKError(
              response,
