...
public class AccountsAssignLargeTrader {
    ...
    <T, U> HttpRequest buildRequest(T request, Class<T> klass, TypeReference<U> typeReference)
      ...
      SerializedBody serializedRequestBody =
          Utils.serializeRequestBody(
              convertedRequest, "assignLargeTraderRequestCreate", "json", false);
  ...
  public static class Sync extends Base
      implements RequestOperation<
          AccountsAssignLargeTraderRequest, AccountsAssignLargeTraderResponse> {
    public Sync(SDKConfiguration sdkConfiguration, Optional<Options> options) {
      super(sdkConfiguration, options);
    }

    private HttpRequest onBuildRequest(AccountsAssignLargeTraderRequest request) throws Exception {
      HttpRequest req =
          buildRequest(
              request,
              AccountsAssignLargeTraderRequest.class,
              new TypeReference<AccountsAssignLargeTraderRequest>() {});
    ...
    @Override
    public HttpResponse<InputStream> doRequest(AccountsAssignLargeTraderRequest request)
        throws Exception {
      Retries retries =
          Retries.builder()
              .action(
    ...
    @Override
    public AccountsAssignLargeTraderResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      AccountsAssignLargeTraderResponse.Builder resBuilder =
          AccountsAssignLargeTraderResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      AccountsAssignLargeTraderResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          LargeTrader out = Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withLargeTrader(out);
          return res;
        } else {
          throw new SDKError(
              response,
