...
public class OrderServiceListAccountOrders {
  ...
  abstract static class Base {
    ...
    <T> HttpRequest buildRequest(T request, Class<T> klass) throws Exception {
      String url =
          Utils.generateURL(
              klass, this.baseUrl, "/trading/v1/accounts/{account_id}/orders", request, null);
  ...
  public static class Sync extends Base
      implements RequestOperation<
          OrderServiceListAccountOrdersRequest, OrderServiceListAccountOrdersResponse> {
    public Sync(SDKConfiguration sdkConfiguration, Optional<Options> options) {
      super(sdkConfiguration, options);
    }

    private HttpRequest onBuildRequest(OrderServiceListAccountOrdersRequest request)
        throws Exception {
      HttpRequest req = buildRequest(request, OrderServiceListAccountOrdersRequest.class);
    ...
    @Override
    public HttpResponse<InputStream> doRequest(OrderServiceListAccountOrdersRequest request)
        throws Exception {
      Retries retries =
          Retries.builder()
              .action(
    ...
    @Override
    public OrderServiceListAccountOrdersResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      OrderServiceListAccountOrdersResponse.Builder resBuilder =
          OrderServiceListAccountOrdersResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      OrderServiceListAccountOrdersResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          ListAccountOrdersResponse out =
              Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withListAccountOrdersResponse(out);
          return res;
        } else {
          throw new SDKError(
              response,
