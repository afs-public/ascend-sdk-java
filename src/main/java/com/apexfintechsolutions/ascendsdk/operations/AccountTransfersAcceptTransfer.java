...
public class AccountTransfersAcceptTransfer {
    ...
    <T, U> HttpRequest buildRequest(T request, Class<T> klass, TypeReference<U> typeReference)
      ...
      SerializedBody serializedRequestBody =
          Utils.serializeRequestBody(
              convertedRequest, "acceptTransferRequestCreate", "json", false);
  ...
  public static class Sync extends Base
      implements RequestOperation<
          AccountTransfersAcceptTransferRequest, AccountTransfersAcceptTransferResponse> {
    public Sync(SDKConfiguration sdkConfiguration, Optional<Options> options) {
      super(sdkConfiguration, options);
    }

    private HttpRequest onBuildRequest(AccountTransfersAcceptTransferRequest request)
        throws Exception {
      HttpRequest req =
          buildRequest(
              request,
              AccountTransfersAcceptTransferRequest.class,
              new TypeReference<AccountTransfersAcceptTransferRequest>() {});
    ...
    @Override
    public HttpResponse<InputStream> doRequest(AccountTransfersAcceptTransferRequest request)
        throws Exception {
      Retries retries =
          Retries.builder()
              .action(
    ...
    @Override
    public AccountTransfersAcceptTransferResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      AccountTransfersAcceptTransferResponse.Builder resBuilder =
          AccountTransfersAcceptTransferResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      AccountTransfersAcceptTransferResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          AcceptTransferResponse out =
              Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withAcceptTransferResponse(out);
          return res;
        } else {
          throw new SDKError(
              response,
