...
public class WireDepositsGetWireDeposit {
  ...
  public static class Sync extends Base
      implements RequestOperation<
          WireDepositsGetWireDepositRequest, WireDepositsGetWireDepositResponse> {
    public Sync(SDKConfiguration sdkConfiguration, Optional<Options> options) {
      super(sdkConfiguration, options);
    }

    private HttpRequest onBuildRequest(WireDepositsGetWireDepositRequest request) throws Exception {
      HttpRequest req = buildRequest(request, WireDepositsGetWireDepositRequest.class);
    ...
    @Override
    public HttpResponse<InputStream> doRequest(WireDepositsGetWireDepositRequest request)
        throws Exception {
      Retries retries =
          Retries.builder()
              .action(
    ...
    @Override
    public WireDepositsGetWireDepositResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      WireDepositsGetWireDepositResponse.Builder resBuilder =
          WireDepositsGetWireDepositResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      WireDepositsGetWireDepositResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          WireDeposit out = Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withWireDeposit(out);
          return res;
        } else {
          throw new SDKError(
              response,
