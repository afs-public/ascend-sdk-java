...
public class AccountsEnrollAccount {
  ...
  abstract static class Base {
    ...
    <T, U> HttpRequest buildRequest(T request, Class<T> klass, TypeReference<U> typeReference)
      ...
      String url =
          Utils.generateURL(
              klass, this.baseUrl, "/accounts/v1/accounts/{account_id}:enroll", request, null);
      ...
      SerializedBody serializedRequestBody =
          Utils.serializeRequestBody(convertedRequest, "enrollAccountRequestCreate", "json", false);
  ...
  public static class Sync extends Base
      implements RequestOperation<AccountsEnrollAccountRequest, AccountsEnrollAccountResponse> {
    public Sync(SDKConfiguration sdkConfiguration, Optional<Options> options) {
      super(sdkConfiguration, options);
    }

    private HttpRequest onBuildRequest(AccountsEnrollAccountRequest request) throws Exception {
      HttpRequest req =
          buildRequest(
              request,
              AccountsEnrollAccountRequest.class,
              new TypeReference<AccountsEnrollAccountRequest>() {});
    ...
    @Override
    public HttpResponse<InputStream> doRequest(AccountsEnrollAccountRequest request)
        throws Exception {
      Retries retries =
          Retries.builder()
              .action(
    ...
    @Override
    public AccountsEnrollAccountResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      AccountsEnrollAccountResponse.Builder resBuilder =
          AccountsEnrollAccountResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      AccountsEnrollAccountResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          EnrollAccountResponse out =
              Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withEnrollAccountResponse(out);
          return res;
        } else {
          throw new SDKError(
              response,
