...
public class ReaderListEventMessages {
  ...
  abstract static class Base {
    ...
    HttpRequest buildRequest(ReaderListEventMessagesRequest request) throws Exception {
      String url = Utils.generateURL(this.baseUrl, "/events/v1/messages");
      HTTPRequest req = new HTTPRequest(url, "GET");
      req.addHeader("Accept", "application/json")
          .addHeader("user-agent", SDKConfiguration.USER_AGENT);

      req.addQueryParams(Utils.getQueryParams(ReaderListEventMessagesRequest.class, request, null));
  ...
  public static class Sync extends Base
      implements RequestOperation<ReaderListEventMessagesRequest, ReaderListEventMessagesResponse> {
    ...
    @Override
    public HttpResponse<InputStream> doRequest(ReaderListEventMessagesRequest request)
        throws Exception {
    ...
    @Override
    public ReaderListEventMessagesResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      ReaderListEventMessagesResponse.Builder resBuilder =
          ReaderListEventMessagesResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      ReaderListEventMessagesResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          ListEventMessagesResponse out =
              Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withListEventMessagesResponse(out);
          return res;
        } else {
          throw new SDKError(
              response,
