...
public class SnapshotsListSnapshots {
  ...
  abstract static class Base {
    ...
    <T> HttpRequest buildRequest(T request, Class<T> klass) throws Exception {
      String url = Utils.generateURL(this.baseUrl, "/analytics/v1/snapshots");
  ...
  public static class Sync extends Base
      implements RequestOperation<SnapshotsListSnapshotsRequest, SnapshotsListSnapshotsResponse> {
    public Sync(SDKConfiguration sdkConfiguration, Optional<Options> options) {
      super(sdkConfiguration, options);
    }

    private HttpRequest onBuildRequest(SnapshotsListSnapshotsRequest request) throws Exception {
      HttpRequest req = buildRequest(request, SnapshotsListSnapshotsRequest.class);
    ...
    @Override
    public HttpResponse<InputStream> doRequest(SnapshotsListSnapshotsRequest request)
        throws Exception {
      Retries retries =
          Retries.builder()
              .action(
                  () -> {
                    ...
                    try {
                      ...
                      if (Utils.statusCodeMatches(
                          httpRes.statusCode(), "400", "403", "4XX", "500", "5XX")) {
    ...
    @Override
    public SnapshotsListSnapshotsResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      SnapshotsListSnapshotsResponse.Builder resBuilder =
          SnapshotsListSnapshotsResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      SnapshotsListSnapshotsResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          ListSnapshotsResponse out =
              Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withListSnapshotsResponse(out);
          return res;
        } else {
          throw new SDKError(
              response,
