...
public class AccountsListNotes {
  ...
  public static class Sync extends Base
      implements RequestOperation<AccountsListNotesRequest, AccountsListNotesResponse> {
    public Sync(SDKConfiguration sdkConfiguration, Optional<Options> options) {
      super(sdkConfiguration, options);
    }

    private HttpRequest onBuildRequest(AccountsListNotesRequest request) throws Exception {
      HttpRequest req = buildRequest(request, AccountsListNotesRequest.class);
    ...
    @Override
    public HttpResponse<InputStream> doRequest(AccountsListNotesRequest request) throws Exception {
      Retries retries =
          Retries.builder()
              .action(
                  () -> {
    ...
    @Override
    public AccountsListNotesResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      AccountsListNotesResponse.Builder resBuilder =
          AccountsListNotesResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      AccountsListNotesResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          ListNotesResponse out =
              Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withListNotesResponse(out);
          return res;
        } else {
          throw new SDKError(
              response,
