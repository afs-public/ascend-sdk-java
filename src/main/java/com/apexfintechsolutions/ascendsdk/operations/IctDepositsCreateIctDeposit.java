...
public class IctDepositsCreateIctDeposit {
  ...
  abstract static class Base {
    ...
    <T, U> HttpRequest buildRequest(T request, Class<T> klass, TypeReference<U> typeReference)
      ...
      SerializedBody serializedRequestBody =
          Utils.serializeRequestBody(convertedRequest, "ictDepositCreate", "json", false);
  ...
  public static class Sync extends Base
      implements RequestOperation<
          IctDepositsCreateIctDepositRequest, IctDepositsCreateIctDepositResponse> {
    public Sync(SDKConfiguration sdkConfiguration, Optional<Options> options) {
      super(sdkConfiguration, options);
    }

    private HttpRequest onBuildRequest(IctDepositsCreateIctDepositRequest request)
        throws Exception {
      HttpRequest req =
          buildRequest(
              request,
              IctDepositsCreateIctDepositRequest.class,
              new TypeReference<IctDepositsCreateIctDepositRequest>() {});
    ...
    @Override
    public HttpResponse<InputStream> doRequest(IctDepositsCreateIctDepositRequest request)
        throws Exception {
      Retries retries =
          Retries.builder()
              .action(
    ...
    @Override
    public IctDepositsCreateIctDepositResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      IctDepositsCreateIctDepositResponse.Builder resBuilder =
          IctDepositsCreateIctDepositResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      IctDepositsCreateIctDepositResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          IctDeposit out = Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withIctDeposit(out);
          return res;
        } else {
          throw new SDKError(
              response,
