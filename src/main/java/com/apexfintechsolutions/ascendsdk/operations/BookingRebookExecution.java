...
public class BookingRebookExecution {
    ...
    <T, U> HttpRequest buildRequest(T request, Class<T> klass, TypeReference<U> typeReference)
      ...
      SerializedBody serializedRequestBody =
          Utils.serializeRequestBody(
              convertedRequest, "rebookExecutionRequestCreate", "json", false);
  ...
  public static class Sync extends Base
      implements RequestOperation<BookingRebookExecutionRequest, BookingRebookExecutionResponse> {
    public Sync(SDKConfiguration sdkConfiguration, Optional<Options> options) {
      super(sdkConfiguration, options);
    }

    private HttpRequest onBuildRequest(BookingRebookExecutionRequest request) throws Exception {
      HttpRequest req =
          buildRequest(
              request,
              BookingRebookExecutionRequest.class,
              new TypeReference<BookingRebookExecutionRequest>() {});
    ...
    @Override
    public HttpResponse<InputStream> doRequest(BookingRebookExecutionRequest request)
        throws Exception {
      Retries retries =
          Retries.builder()
              .action(
    ...
    @Override
    public BookingRebookExecutionResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      BookingRebookExecutionResponse.Builder resBuilder =
          BookingRebookExecutionResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      BookingRebookExecutionResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          RebookExecutionResponse out =
              Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withRebookExecutionResponse(out);
          return res;
        } else {
          throw new SDKError(
              response,
