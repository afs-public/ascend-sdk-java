...
public class FeesCreateFee {
  ...
  abstract static class Base {
    ...
    <T, U> HttpRequest buildRequest(T request, Class<T> klass, TypeReference<U> typeReference)
      ...
      String url =
          Utils.generateURL(
              klass, this.baseUrl, "/transfers/v1/accounts/{account_id}/fees", request, null);
      ...
      SerializedBody serializedRequestBody =
          Utils.serializeRequestBody(convertedRequest, "transfersFeeCreate", "json", false);
  ...
  public static class Sync extends Base
      implements RequestOperation<FeesCreateFeeRequest, FeesCreateFeeResponse> {
    public Sync(SDKConfiguration sdkConfiguration, Optional<Options> options) {
      super(sdkConfiguration, options);
    }

    private HttpRequest onBuildRequest(FeesCreateFeeRequest request) throws Exception {
      HttpRequest req =
          buildRequest(
              request, FeesCreateFeeRequest.class, new TypeReference<FeesCreateFeeRequest>() {});
    ...
    @Override
    public HttpResponse<InputStream> doRequest(FeesCreateFeeRequest request) throws Exception {
      Retries retries =
          Retries.builder()
              .action(
                  () -> {
    ...
    @Override
    public FeesCreateFeeResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      FeesCreateFeeResponse.Builder resBuilder =
          FeesCreateFeeResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      FeesCreateFeeResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          TransfersFee out = Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withTransfersFee(out);
          return res;
        } else {
          throw new SDKError(
              response,
