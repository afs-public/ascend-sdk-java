...
public class AccountTransfersCreateTransfer {
  ...
  abstract static class Base {
    ...
    <T, U> HttpRequest buildRequest(T request, Class<T> klass, TypeReference<U> typeReference)
      ...
      SerializedBody serializedRequestBody =
          Utils.serializeRequestBody(convertedRequest, "transferCreate", "json", false);
  ...
  public static class Sync extends Base
      implements RequestOperation<
          AccountTransfersCreateTransferRequest, AccountTransfersCreateTransferResponse> {
    public Sync(SDKConfiguration sdkConfiguration, Optional<Options> options) {
      super(sdkConfiguration, options);
    }

    private HttpRequest onBuildRequest(AccountTransfersCreateTransferRequest request)
        throws Exception {
      HttpRequest req =
          buildRequest(
              request,
              AccountTransfersCreateTransferRequest.class,
              new TypeReference<AccountTransfersCreateTransferRequest>() {});
    ...
    @Override
    public HttpResponse<InputStream> doRequest(AccountTransfersCreateTransferRequest request)
        throws Exception {
      Retries retries =
          Retries.builder()
              .action(
    ...
    @Override
    public AccountTransfersCreateTransferResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      AccountTransfersCreateTransferResponse.Builder resBuilder =
          AccountTransfersCreateTransferResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      AccountTransfersCreateTransferResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          AcatsTransfer out = Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withAcatsTransfer(out);
          return res;
        } else {
          throw new SDKError(
              response,
