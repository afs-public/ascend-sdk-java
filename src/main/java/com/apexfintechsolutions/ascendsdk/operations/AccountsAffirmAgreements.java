...
public class AccountsAffirmAgreements {
    ...
    <T, U> HttpRequest buildRequest(T request, Class<T> klass, TypeReference<U> typeReference)
      ...
      SerializedBody serializedRequestBody =
          Utils.serializeRequestBody(
              convertedRequest, "affirmAgreementsRequestCreate", "json", false);
  ...
  public static class Sync extends Base
      implements RequestOperation<
          AccountsAffirmAgreementsRequest, AccountsAffirmAgreementsResponse> {
    public Sync(SDKConfiguration sdkConfiguration, Optional<Options> options) {
      super(sdkConfiguration, options);
    }

    private HttpRequest onBuildRequest(AccountsAffirmAgreementsRequest request) throws Exception {
      HttpRequest req =
          buildRequest(
              request,
              AccountsAffirmAgreementsRequest.class,
              new TypeReference<AccountsAffirmAgreementsRequest>() {});
    ...
    @Override
    public HttpResponse<InputStream> doRequest(AccountsAffirmAgreementsRequest request)
        throws Exception {
      Retries retries =
          Retries.builder()
              .action(
    ...
    @Override
    public AccountsAffirmAgreementsResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      AccountsAffirmAgreementsResponse.Builder resBuilder =
          AccountsAffirmAgreementsResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      AccountsAffirmAgreementsResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          AffirmAgreementsResponse out =
              Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withAffirmAgreementsResponse(out);
          return res;
        } else {
          throw new SDKError(
              response,
