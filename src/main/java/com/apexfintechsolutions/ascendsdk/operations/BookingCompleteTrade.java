...
public class BookingCompleteTrade {
  ...
  abstract static class Base {
    ...
    <T, U> HttpRequest buildRequest(T request, Class<T> klass, TypeReference<U> typeReference)
      ...
      SerializedBody serializedRequestBody =
          Utils.serializeRequestBody(convertedRequest, "completeTradeRequestCreate", "json", false);
  ...
  public static class Sync extends Base
      implements RequestOperation<BookingCompleteTradeRequest, BookingCompleteTradeResponse> {
    public Sync(SDKConfiguration sdkConfiguration, Optional<Options> options) {
      super(sdkConfiguration, options);
    }

    private HttpRequest onBuildRequest(BookingCompleteTradeRequest request) throws Exception {
      HttpRequest req =
          buildRequest(
              request,
              BookingCompleteTradeRequest.class,
              new TypeReference<BookingCompleteTradeRequest>() {});
    ...
    @Override
    public HttpResponse<InputStream> doRequest(BookingCompleteTradeRequest request)
        throws Exception {
      Retries retries =
          Retries.builder()
              .action(
    ...
    @Override
    public BookingCompleteTradeResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      BookingCompleteTradeResponse.Builder resBuilder =
          BookingCompleteTradeResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      BookingCompleteTradeResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          CompleteTradeResponse out =
              Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withCompleteTradeResponse(out);
          return res;
        } else {
          throw new SDKError(
              response,
