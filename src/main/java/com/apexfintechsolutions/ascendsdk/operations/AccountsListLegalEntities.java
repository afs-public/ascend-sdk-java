...
public class AccountsListLegalEntities {
  ...
  abstract static class Base {
    ...
    <T> HttpRequest buildRequest(T request, Class<T> klass) throws Exception {
      String url = Utils.generateURL(this.baseUrl, "/accounts/v1/legalEntities");
  ...
  public static class Sync extends Base
      implements RequestOperation<
          AccountsListLegalEntitiesRequest, AccountsListLegalEntitiesResponse> {
    public Sync(SDKConfiguration sdkConfiguration, Optional<Options> options) {
      super(sdkConfiguration, options);
    }

    private HttpRequest onBuildRequest(AccountsListLegalEntitiesRequest request) throws Exception {
      HttpRequest req = buildRequest(request, AccountsListLegalEntitiesRequest.class);
    ...
    @Override
    public HttpResponse<InputStream> doRequest(AccountsListLegalEntitiesRequest request)
        throws Exception {
      Retries retries =
          Retries.builder()
              .action(
    ...
    @Override
    public AccountsListLegalEntitiesResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      AccountsListLegalEntitiesResponse.Builder resBuilder =
          AccountsListLegalEntitiesResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      AccountsListLegalEntitiesResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          ListLegalEntitiesResponse out =
              Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withListLegalEntitiesResponse(out);
          return res;
        } else {
          throw new SDKError(
              response,
