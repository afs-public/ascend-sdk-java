...
public class BookingCancelTrade {
  ...
  abstract static class Base {
    ...
    <T, U> HttpRequest buildRequest(T request, Class<T> klass, TypeReference<U> typeReference)
      ...
      SerializedBody serializedRequestBody =
          Utils.serializeRequestBody(convertedRequest, "cancelTradeRequestCreate", "json", false);
  ...
  public static class Sync extends Base
      implements RequestOperation<BookingCancelTradeRequest, BookingCancelTradeResponse> {
    public Sync(SDKConfiguration sdkConfiguration, Optional<Options> options) {
      super(sdkConfiguration, options);
    }

    private HttpRequest onBuildRequest(BookingCancelTradeRequest request) throws Exception {
      HttpRequest req =
          buildRequest(
              request,
              BookingCancelTradeRequest.class,
              new TypeReference<BookingCancelTradeRequest>() {});
    ...
    @Override
    public HttpResponse<InputStream> doRequest(BookingCancelTradeRequest request) throws Exception {
      Retries retries =
          Retries.builder()
              .action(
                  () -> {
    ...
    @Override
    public BookingCancelTradeResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      BookingCancelTradeResponse.Builder resBuilder =
          BookingCancelTradeResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      BookingCancelTradeResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          CancelTradeResponse out =
              Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withCancelTradeResponse(out);
          return res;
        } else {
          throw new SDKError(
              response,
