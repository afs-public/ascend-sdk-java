...
public class AccountsUpdateLegalEntity {
  ...
  abstract static class Base {
    ...
    <T, U> HttpRequest buildRequest(T request, Class<T> klass, TypeReference<U> typeReference)
      ...
      String url =
          Utils.generateURL(
              klass, this.baseUrl, "/accounts/v1/legalEntities/{legalEntity_id}", request, null);
      ...
      SerializedBody serializedRequestBody =
          Utils.serializeRequestBody(convertedRequest, "legalEntityUpdate", "json", false);
  ...
  public static class Sync extends Base
      implements RequestOperation<
          AccountsUpdateLegalEntityRequest, AccountsUpdateLegalEntityResponse> {
    public Sync(SDKConfiguration sdkConfiguration, Optional<Options> options) {
      super(sdkConfiguration, options);
    }

    private HttpRequest onBuildRequest(AccountsUpdateLegalEntityRequest request) throws Exception {
      HttpRequest req =
          buildRequest(
              request,
              AccountsUpdateLegalEntityRequest.class,
              new TypeReference<AccountsUpdateLegalEntityRequest>() {});
    ...
    @Override
    public HttpResponse<InputStream> doRequest(AccountsUpdateLegalEntityRequest request)
        throws Exception {
      Retries retries =
          Retries.builder()
              .action(
    ...
    @Override
    public AccountsUpdateLegalEntityResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      AccountsUpdateLegalEntityResponse.Builder resBuilder =
          AccountsUpdateLegalEntityResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      AccountsUpdateLegalEntityResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          LegalEntity out = Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withLegalEntity(out);
          return res;
        } else {
          throw new SDKError(
              response,
