...
public class BookingCreateExecution {
  ...
  abstract static class Base {
    ...
    <T, U> HttpRequest buildRequest(T request, Class<T> klass, TypeReference<U> typeReference)
      ...
      SerializedBody serializedRequestBody =
          Utils.serializeRequestBody(convertedRequest, "executionCreate", "json", false);
  ...
  public static class Sync extends Base
      implements RequestOperation<BookingCreateExecutionRequest, BookingCreateExecutionResponse> {
    public Sync(SDKConfiguration sdkConfiguration, Optional<Options> options) {
      super(sdkConfiguration, options);
    }

    private HttpRequest onBuildRequest(BookingCreateExecutionRequest request) throws Exception {
      HttpRequest req =
          buildRequest(
              request,
              BookingCreateExecutionRequest.class,
              new TypeReference<BookingCreateExecutionRequest>() {});
    ...
    @Override
    public HttpResponse<InputStream> doRequest(BookingCreateExecutionRequest request)
        throws Exception {
      Retries retries =
          Retries.builder()
              .action(
                  () -> {
                    ...
                    try {
                      ...
                      if (Utils.statusCodeMatches(
                          httpRes.statusCode(), "400", "403", "4XX", "500", "503", "504", "5XX")) {
    ...
    @Override
    public BookingCreateExecutionResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      BookingCreateExecutionResponse.Builder resBuilder =
          BookingCreateExecutionResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      BookingCreateExecutionResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          Execution out = Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withExecution(out);
          return res;
        } else {
          throw new SDKError(
              response,
