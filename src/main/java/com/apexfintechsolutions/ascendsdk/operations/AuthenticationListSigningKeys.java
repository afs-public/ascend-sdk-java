...
public class AuthenticationListSigningKeys {

  abstract static class Base {
    final SDKConfiguration sdkConfiguration;
    final String baseUrl;
    final AuthenticationListSigningKeysSecurity security;
    final SecuritySource securitySource;
    final HTTPClient client;

    public Base(SDKConfiguration sdkConfiguration, AuthenticationListSigningKeysSecurity security) {
    ...
    HttpRequest buildRequest(AuthenticationListSigningKeysRequest request) throws Exception {
      String url = Utils.generateURL(this.baseUrl, "/iam/v1/keys");
  ...
  public static class Sync extends Base
      implements RequestOperation<
          AuthenticationListSigningKeysRequest, AuthenticationListSigningKeysResponse> {
    public Sync(SDKConfiguration sdkConfiguration, AuthenticationListSigningKeysSecurity security) {
    ...
    @Override
    public HttpResponse<InputStream> doRequest(AuthenticationListSigningKeysRequest request)
        throws Exception {
      HttpRequest r = buildRequest(request);
      HttpResponse<InputStream> httpRes;
      try {
        httpRes = client.send(r);
        if (Utils.statusCodeMatches(httpRes.statusCode(), "4XX", "500", "5XX")) {
    ...
    @Override
    public AuthenticationListSigningKeysResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      AuthenticationListSigningKeysResponse.Builder resBuilder =
          AuthenticationListSigningKeysResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      AuthenticationListSigningKeysResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          ListSigningKeysResponse out =
              Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withListSigningKeysResponse(out);
          return res;
        } else {
          throw new SDKError(
              response,
