...
public class CashJournalsCancelCashJournal {
    ...
    <T, U> HttpRequest buildRequest(T request, Class<T> klass, TypeReference<U> typeReference)
      ...
      SerializedBody serializedRequestBody =
          Utils.serializeRequestBody(
              convertedRequest, "cancelCashJournalRequestCreate", "json", false);
  ...
  public static class Sync extends Base
      implements RequestOperation<
          CashJournalsCancelCashJournalRequest, CashJournalsCancelCashJournalResponse> {
    public Sync(SDKConfiguration sdkConfiguration, Optional<Options> options) {
      super(sdkConfiguration, options);
    }

    private HttpRequest onBuildRequest(CashJournalsCancelCashJournalRequest request)
        throws Exception {
      HttpRequest req =
          buildRequest(
              request,
              CashJournalsCancelCashJournalRequest.class,
              new TypeReference<CashJournalsCancelCashJournalRequest>() {});
    ...
    @Override
    public HttpResponse<InputStream> doRequest(CashJournalsCancelCashJournalRequest request)
        throws Exception {
      Retries retries =
          Retries.builder()
              .action(
    ...
    @Override
    public CashJournalsCancelCashJournalResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      CashJournalsCancelCashJournalResponse.Builder resBuilder =
          CashJournalsCancelCashJournalResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      CashJournalsCancelCashJournalResponse res = resBuilder.build();
