...
public class AccountsCloseAccount {
  ...
  abstract static class Base {
    ...
    <T, U> HttpRequest buildRequest(T request, Class<T> klass, TypeReference<U> typeReference)
      ...
      String url =
          Utils.generateURL(
              klass, this.baseUrl, "/accounts/v1/accounts/{account_id}:close", request, null);
      ...
      SerializedBody serializedRequestBody =
          Utils.serializeRequestBody(convertedRequest, "closeAccountRequestCreate", "json", false);
  ...
  public static class Sync extends Base
      implements RequestOperation<AccountsCloseAccountRequest, AccountsCloseAccountResponse> {
    public Sync(SDKConfiguration sdkConfiguration, Optional<Options> options) {
      super(sdkConfiguration, options);
    }

    private HttpRequest onBuildRequest(AccountsCloseAccountRequest request) throws Exception {
      HttpRequest req =
          buildRequest(
              request,
              AccountsCloseAccountRequest.class,
              new TypeReference<AccountsCloseAccountRequest>() {});
    ...
    @Override
    public HttpResponse<InputStream> doRequest(AccountsCloseAccountRequest request)
        throws Exception {
      Retries retries =
          Retries.builder()
              .action(
    ...
    @Override
    public AccountsCloseAccountResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      AccountsCloseAccountResponse.Builder resBuilder =
          AccountsCloseAccountResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      AccountsCloseAccountResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          Account out = Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withAccount(out);
          return res;
        } else {
          throw new SDKError(
