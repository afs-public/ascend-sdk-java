...
public class AssetsGetAsset {
  ...
  abstract static class Base {
    ...
    <T> HttpRequest buildRequest(T request, Class<T> klass) throws Exception {
      String url =
          Utils.generateURL(klass, this.baseUrl, "/assets/v1/assets/{asset_id}", request, null);
  ...
  public static class Sync extends Base
      implements RequestOperation<AssetsGetAssetRequest, AssetsGetAssetResponse> {
    public Sync(SDKConfiguration sdkConfiguration, Optional<Options> options) {
      super(sdkConfiguration, options);
    }

    private HttpRequest onBuildRequest(AssetsGetAssetRequest request) throws Exception {
      HttpRequest req = buildRequest(request, AssetsGetAssetRequest.class);
    ...
    @Override
    public HttpResponse<InputStream> doRequest(AssetsGetAssetRequest request) throws Exception {
      Retries retries =
          Retries.builder()
              .action(
                  () -> {
    ...
    @Override
    public AssetsGetAssetResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      AssetsGetAssetResponse.Builder resBuilder =
          AssetsGetAssetResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      AssetsGetAssetResponse res = resBuilder.build();
