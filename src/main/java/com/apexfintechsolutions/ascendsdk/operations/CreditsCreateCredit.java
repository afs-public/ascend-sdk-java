...
public class CreditsCreateCredit {
  ...
  abstract static class Base {
    ...
    <T, U> HttpRequest buildRequest(T request, Class<T> klass, TypeReference<U> typeReference)
      ...
      String url =
          Utils.generateURL(
              klass, this.baseUrl, "/transfers/v1/accounts/{account_id}/credits", request, null);
      ...
      SerializedBody serializedRequestBody =
          Utils.serializeRequestBody(convertedRequest, "transfersCreditCreate", "json", false);
  ...
  public static class Sync extends Base
      implements RequestOperation<CreditsCreateCreditRequest, CreditsCreateCreditResponse> {
    public Sync(SDKConfiguration sdkConfiguration, Optional<Options> options) {
      super(sdkConfiguration, options);
    }

    private HttpRequest onBuildRequest(CreditsCreateCreditRequest request) throws Exception {
      HttpRequest req =
          buildRequest(
              request,
              CreditsCreateCreditRequest.class,
              new TypeReference<CreditsCreateCreditRequest>() {});
    ...
    @Override
    public HttpResponse<InputStream> doRequest(CreditsCreateCreditRequest request)
        throws Exception {
      Retries retries =
          Retries.builder()
              .action(
    ...
    @Override
    public CreditsCreateCreditResponse handleResponse(HttpResponse<InputStream> response)
        throws Exception {
      String contentType =
          response.headers().firstValue("Content-Type").orElse("application/octet-stream");
      CreditsCreateCreditResponse.Builder resBuilder =
          CreditsCreateCreditResponse.builder()
              .contentType(contentType)
              .statusCode(response.statusCode())
              .rawResponse(response);

      CreditsCreateCreditResponse res = resBuilder.build();

      if (Utils.statusCodeMatches(response.statusCode(), "200")) {
        if (Utils.contentTypeMatches(contentType, "application/json")) {
          TransfersCredit out = Utils.mapper().readValue(response.body(), new TypeReference<>() {});
          res.withTransfersCredit(out);
          return res;
        } else {
          throw new SDKError(
              response,
